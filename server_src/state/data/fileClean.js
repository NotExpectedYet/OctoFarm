...
class FileClean {
  ...
  #logger = new Logger("OctoFarm-InformationCleaning");

  constructor({}) {}

  returnFiles(p) {
    return this.#cleanFileList[p];
  }
   ...
   * @returns {*[]}
   */
  getUnits(filamentSelection, fileLength) {
    const strings = [];
    const lengthArray = [];
    const weightArray = [];
    if (!!fileLength) {
   ...
   * @returns {*[]}
   */
  getCost(filamentSelection, units) {
